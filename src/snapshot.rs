...
pub struct HistoryIterator<'a> {
	...
	// Timestamps erased for the current user_key. A version whose timestamp is
	// in this set has been erased and must be skipped (reads fall through to the
	// previous version). Erase markers always have a higher seq than the version
	// they erase, so they are encountered first during forward iteration.
	erased_timestamps: Vec<u64>,
...
impl<'a> HistoryIterator<'a> {
	...
	fn skip_to_valid_forward(&mut self) -> Result<bool> {
		while self.inner_valid() {
			...
			let (user_key_vec, seq_num, timestamp, is_hard_delete, is_replace, is_tombstone, is_erase) = {
			...
			// Erase marker: record its timestamp and consume it. The marker has
			// no value, so it must never be yielded. The version it erases (at
			// the same timestamp) appears later in the chain and is skipped below.
			if is_erase {
				self.erased_timestamps.push(timestamp);
				self.inner_next()?;
				continue;
			}

			// Skip versions whose timestamp was erased - reads fall through to
			// the previous (older) version.
			if self.erased_timestamps.contains(&timestamp) {
				self.inner_next()?;
				continue;
			}
	...
	fn collect_user_key_backward(&mut self) -> Result<bool> {
		...
		// Timestamps with an erase marker: the version at each is dropped and
		// reads fall through to the previous version.
		let mut erased_timestamps: Vec<u64> = Vec::new();
		...
		while self.inner_valid() {
			...
			if visible && in_ts_range {
				if key_ref.is_erase() {
					// Erase marker: record its timestamp and drop the marker (it
					// has no value and must never be yielded).
					erased_timestamps.push(timestamp);
				} else {
		...
		// Drop versions whose timestamp was erased.
		versions.retain(|v| !erased_timestamps.contains(&v.timestamp));

		if versions.is_empty() {
			// This user_key yielded nothing (no visible versions, or every
			// version was erased). The inner iterator is already positioned at
			// the next (lower) user_key, so continue rather than halting.
			return self.collect_user_key_backward();
		}
